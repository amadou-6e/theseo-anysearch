...
- Keep already integrated non-encoder code on `exp/perception-encoder`.
  Selectively port reusable capabilities to `develop` through reviewed PRs,
  validate them against current `develop`, then merge updated `develop` forward
  into `exp/perception-encoder`. Do not remove or revert that code for branch
  cleanup, force-push, rewrite public history, or alter frozen run identities.
  The encoder-only scope governs future task placement, not inherited code.
...
- Keep task ownership aligned with
  [the perception-encoder work plan](https://github.com/amadou-6e/theseo-anysearch/blob/117bd8a/docs/perception-encoder-experiment-work-plan.md).
