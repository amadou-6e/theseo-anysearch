...
The native `segment_countdown_goal` reward is sparse. For a segment with exact shortest length `L`, reaching it on segment step `s` returns `max(L + additional_budget - s, minimum_reward)`. With `additional_budget: 10.0`, every shortest-path segment earns 10 regardless of its length. The segment step counter and budget reset immediately when the next waypoint becomes active; no step, collision, shaping, invalid-action, or construction reward is added.
