use anysearch_extension::{anysearch_reward, RewardContext, RewardResult};

fn reward_value(
    goal_reached: bool,
    segment_length: u64,
    segment_step: u64,
    additional_budget: f64,
    minimum_reward: f64,
) -> f64 {
    if !goal_reached {
        return 0.0;
    }
    (segment_length as f64 + additional_budget - segment_step as f64).max(minimum_reward)
}

#[anysearch_reward]
pub fn segment_countdown_goal(context: &RewardContext) -> RewardResult {
    let additional_budget = context.parameters["additional_budget"]
        .as_f64()
        .expect("additional_budget must be numeric");
    let minimum_reward = context.parameters["minimum_reward"]
        .as_f64()
        .expect("minimum_reward must be numeric");
    let reward = reward_value(
        context.goal_reached,
        context.segment_length,
        context.segment_step,
        additional_budget,
        minimum_reward,
    );
    RewardResult::replace(reward).with_component("segment_countdown_goal", reward)
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn resets_the_reward_budget_per_segment() {
        assert_eq!(reward_value(false, 4, 1, 10.0, 1.0), 0.0);
        assert_eq!(reward_value(true, 4, 4, 10.0, 1.0), 10.0);
        assert_eq!(reward_value(true, 1, 1, 10.0, 1.0), 10.0);
        assert_eq!(reward_value(true, 4, 13, 10.0, 1.0), 1.0);
    }
}
