...
pub struct RewardResultV1 {
    ...
    pub components: [RewardComponentV1; MAX_REWARD_COMPONENTS],
}
...
impl From<&RewardContextV1> for RewardContext {
    fn from(value: &RewardContextV1) -> Self {
        Self {
            step: value.step,
...
/// Convert one ergonomic reward function into the stable v1 ABI response.
///
/// # Safety
/// `context` and `result` must point to writable/readable v1 ABI structures.
pub unsafe fn export_reward_v1(
    context: *const RewardContextV1,
    result: *mut RewardResultV1,
    ...
    if context.abi_version != ABI_VERSION
        || context.struct_size as usize != std::mem::size_of::<RewardContextV1>()
    {
        return 2;
    }
    ...
    raw.struct_size = std::mem::size_of::<RewardResultV1>() as u32;
...
mod tests {
    ...
    #[test]
    fn attribute_generates_and_encodes_v1_export() {
        ...
        let mut result: RewardResultV1 = unsafe { std::mem::zeroed() };

        let status = unsafe { anysearch_reward_collision_penalty_v1(&context, &mut result) };
