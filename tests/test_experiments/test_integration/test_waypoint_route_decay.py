...
def test_native_decay_reward_continues_across_waypoint_route() -> None:
    experiment = Path("usage", "experiments", "tune", "waypoint_route_decay")
    ...
    env = VoxelEnv({
        ...
        "custom_reward": "time_decayed_goal",
    ...
    try:
        ...
        _, reward, terminated, truncated, info = env.step(positive_x)
        assert reward == pytest.approx(148.0)
        assert not terminated
        assert not truncated
        assert info["waypoint_reached"]
        assert not info["goal_reached"]
        assert info["reward_breakdown"] == {"time_decayed_goal": pytest.approx(148.0)}

        _, reward, terminated, truncated, info = env.step(positive_x)
        assert reward == pytest.approx(147.0)
        assert terminated
        assert not truncated
